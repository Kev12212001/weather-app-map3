...
def map4_view():
    ...
    for data in weather_data_list:
        ...
        if weather_data and isinstance(weather_data, dict):
            if 'dewpt' in weather_data and weather_data['dewpt'] is not None:
                dewpt_f = (weather_data['dewpt'] * 9/5) + 32
        coords = locations[city_name]
        lats.append(coords['lat'])
        lons.append(coords['lon'])
        dewpoints.append(dewpt_f)
    ...
    for city, coords in locations.items():
        ...
        if city == "Chicago":
            ...
            ax.text(LON, LAT + 0.01, f"{dewpt_f:.1f}°F", color='white', fontweight='bold', ha='center', fontsize=10,
        ...
        if city == "Des Plaines":
            ax.text(LON + 0.005, LAT + 0.0, f"{city}",
                    fontsize=8, color='white', zorder=1.8, transform=ccrs.PlateCarree(),
                    bbox=dict(facecolor='black', alpha=0.7, edgecolor='none', boxstyle='round,pad=0.1'))
            ax.text(LON + 0.02, LAT + 0.01, f"{dewpt_f:.1f}°F", color='white', fontweight='bold', ha='center', fontsize=10,
                transform=ccrs.PlateCarree(), bbox=dict(facecolor='black', alpha=0.7, edgecolor='none', boxstyle='round,pad=0.1'))
    ...
    ax.set_title(f"Surface Dewpoint Observations as of {formatted_time}")
